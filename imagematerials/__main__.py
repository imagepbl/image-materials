...
def simulate_stocks(prep_data):
    ...
    material_type = prism.Dimension("material", coords=[str(x) for x in prep_data["material_fractions"].coords["material"].values])
    ...
    stock_by_cohort_materials = xr.DataArray(0.0, dims=("Time", "Cohort", "Region", "Type", "material"),
                                   coords={"Time": total_stocks.coords["Time"],
                                           ...
                                           "Region": total_stocks.coords["Region"],
                                           "Type": total_stocks.coords["Type"],
                                           "material": prep_data["material_fractions"].coords["material"]})
    ...
    class Stocks(prism.Model):
        ...
        def compute_values(self, time: prism.Time):
            ...
            self.inflow_materials[t] = self.inflow[t]*self.material_fractions.sel(Cohort=t).drop_vars("Cohort")*self.weights.sel(Cohort=t).drop_vars("Cohort")
            self.outflow_by_cohort_materials[t] = self.outflow_by_cohort[t]*self.material_fractions*self.weights
            self.stock_by_cohort_materials.loc[t] = self.stock_by_cohort.loc[t]*self.material_fractions*self.weights
            # self.inflow_materials[t] = material_computation(self.inflow[t], self.material_fractions, self.weights)
    ...
    model = Stocks(timeline, start_simulation=start_simulation, survival_matrix=survival_matrix,
                   ...
                   weights=prep_data["vehicle_weights"])
