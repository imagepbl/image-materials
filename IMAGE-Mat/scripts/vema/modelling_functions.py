...
def inflow_outflow_typical_np(stock: pd.DataFrame,
    ...
    for vtype in vtype_list:
        stock_by_vtype.loc[:, idx[vtype, :]] = stock.mul(
            stock_share.loc[:, idx[vtype, :]])
    ...
    pd_inflow_mat = pd.DataFrame(
        inflow_mat.transpose(
            ...
            (length_materials * length_time), REGIONS), index=index, columns=range(
                1, len(
                    inflow[0]) + 1))
    pd_outflow_mat = pd.DataFrame(outflow_mat.transpose(0, 2, 1).reshape((length_materials * length_time) REGIONS), 
...
def preprocess_to_xarray(preprocessing_results, unit_mapping):
    ...
    for key, df in preprocessing_results.items():
        ...
        if key in unit_mapping:
            ds = ds.assign_attrs({name: unit for name,
                                  unit in unit_mapping[key]['columns'].items()})
            ds = ds.pint.quantify()
