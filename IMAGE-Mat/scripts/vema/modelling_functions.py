...
def interpolate(original: pd.DataFrame, 
    ...
    if change == 'no':
        ...
        reindexed_filled = reindexed.interpolate(kind='linear',
...
def inflow_outflow_dynamic_np(stock: pd.DataFrame, 
    ...
    if distribution == 'FoldedNormal':
        ...
        # the mean times the multiplier (i.e. the standard deviation as a fraction of the mean)
    else:
        fact2_list = fact2
    ...
    for vtype in vtype_list:
        if stock_share.loc[:, idx[vtype, :]].sum().sum() > 0.0001:
            dsm_inflow, dsm_outflow_coh, dsm_stock_coh = inflow_outflow_dynamic_np(
                stock_by_vtype.loc[:, idx[vtype, :]].to_numpy(), fact1, fact2,
                distribution)
...
def nr_by_cohorts_to_materials_simple_np(inflow, outflow_cohort, stock_cohort,
                                         weight, composition):
    """
    ...
    for material in range(0, len(composition.columns)):
        ...
        if composition.iloc[:, material].sum() > 0.001:
            for region in range(0, len(inflow[0])):
                ...
                inflow_mat[material, region, :] = (inflow[:, region] * weight) \
                    * composition_used
    ...
    pd_inflow_mat = pd.DataFrame(
        inflow_mat.transpose(0, 2, 1).reshape((length_materials * length_time),
                                              REGIONS), index=index,
        columns=range(1, len(inflow[0]) + 1))
    pd_outflow_mat = pd.DataFrame(
        outflow_mat.transpose(0, 2, 1).reshape((length_materials * length_time),
                                               REGIONS), index=index,
        columns=range(1, len(inflow[0]) + 1))
    pd_stock_mat = pd.DataFrame(
        stock_mat.transpose(0, 2, 1).reshape((length_materials * length_time),
                                             REGIONS), index=index,
        columns=range(1, len(inflow[0]) + 1))
...
def nr_by_cohorts_to_materials_typical_np(inflow, outflow_cohort, stock_cohort, weight, composition):
    ...
    # columns.unique() keeps the original order, which is important here (same for materials)
