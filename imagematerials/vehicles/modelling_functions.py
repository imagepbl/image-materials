...
def interpolate(original: pd.DataFrame,
    ...
    time-series initiated with 0 as a value)
    2) Optionally, it allows to derive a trend based on the originally specified data and apply that trend into the
    future model years

    Assumptions:
    Assumes: that the time dimension is the only dimension over which the interpolation is applied & that time is the
    (single-level) index of the DataFrame
    ...
    original : pd.DataFrame
        Original data that requires interpolation over the time dimension, which is assumed to be the (single-level) 
        index
    first_year : if given pd.DataFrame, optional (if not given default is an empty list)
        data describing the first year of operation. If given, it will assume values were 0 in the first year of 
        operation and do the interpolation afterwards. The default is []: pd.DataFrame.
    change : str, optional
        argument to fix future values at the last known value (change=no) or to derive a trend based on the original 
        data and apply it as an indexed growth after the last known year. The default is 'no'.
    ...
    # reindexing from model start year to end year (i.e. the entire model period, adding NaN values for non-existing
    # years)
    ...
    # change='yes' means the future values are based on the trend in the years
    # that were known in the original input
    if change == 'no':
        # interpolate both ways, meaning that linear interpolation is performed between years with data & is kept
        # constant at the first/last known data point before/after
        reindexed_filled = reindexed.interpolate(
            kind='linear', limit_direction='both')
    else:
        # if change = 'yes'
        # interpolate, but not after the last known year
        reindexed_filled = reindexed.interpolate(
            kind='linear', limit_direction='backward')

        # first determine the growth rate based on the historic series
        # (start-to-end)
        ...
        for row in range(end + 1, END_YEAR + 1):
            reindexed_filled.loc[row] = reindexed_filled.loc[row -
                                                             1].values * (1 + growthrate)
...
def tkms_to_nr_of_vehicles_fixed(tera_tkms, mileage, load, loadfactor):
    """
    This function translates ton kilometers (by year & by region) to nr of vehicles (same dimms) using fixed indicators 
...
def scenario_change(df, base_year, target_year, change, implementation_rate, data_type = None, steepness=0.5):
    ...
    Parameters:
        ...
        change (dict): A dictionary mapping modes to percentage increases (e.g., {'Cars': 20} for +20%).
    ...
    Returns:
        pd.DataFrame: A new DataFrame with updated values for each year between base_year and target_year, 
                      and interpolated values where necessary.

    Raises:
        ValueError: If the implementation method is unsupported or if the specified column is not found in the DataFrame.
    """
...
def apply_change_per_region(df, base_year, target_year, increase, implementation_rate, data_type = None, steepness=0.5):
    """
    ...
    Parameters:
        ...
        base_year (int): The starting year for the change.
        target_year (int): The year by which the full change should be achieved.
        increase (float): The percentage increase to apply to all regions (e.g., 10 for +10%).
        implementation_rate (str): The implementation method; one of 'linear', 'immediate', or 's-curve'.
        data_type (str): Indicates what kind of data is being modified; one of 'lifetime' or 'mileages'.
        steepness (float, optional): Steepness parameter for the 's-curve' implementation; default is 0.5.
...
#     This DYNAMIC variant of the DSM calculates & returns the full unadjusted (cohort specific) matrix (YES, very memory
#     intensive, but neccesary for dynamic assesmment, i.e. changing vehicle weights & compositions)
#     """
...
#     # define mean (fact1) & standard deviation (fact2) (applied accross all regions) (For Weibull fact1 = shape &
#     # fact2 = scale)

#     # fact 1 is a list with the mean lifetime (for Weibull: it's the shape parameter), given for every timestep
#     # (e.g. 1900-2100), needed for the DSM as it allows to change lifetime for different cohort
#     # fact 2 is a list with the standarddeviation as a fraction of the mean lifetime (for Weibull: it's the scale
#     # parameter)
#     if distribution == 'FoldedNormal':
#         fact2_list = fact1.mul(fact2)
#         # for the (Folded) normal distribution, the standard_deviation is calculated as
#         # the mean times the multiplier (i.e. the standard deviation as a
...
#         if distribution == 'FoldedNormal':
#             DSMforward = DSM(t=np.arange(0, len(stock[:, region]), 1),
#                              s=stock[:, region],
...
#         # run the DSM, to give 3 outputs: stock_by_cohort, outflow_by_cohort &
#         # inflow_per_year
...
# Pre-calculate the inflow (aka. market-)share corresponding to the
# (known) share of vehicles in stock (from IMAGE)
...
#     IN: stock as a dataframe (nr of vehicles by year, region and type), lifetime (in years), stdev_mult (multiplier to
#     derive the standard deviation from the mean)
#     OUT: 3 dataframes of stock, inflow & outflow (nr of vehicles by year^2 & region & type)

#     This TYPICAL variant of the DSM calculates & returns the full unadjusted (cohort specific) matrix (time*time) for
#     every region AND vehicle type (YES, very memory intensive, but neccesary for dynamic assesmment, i.e. changing
#     vehicle weights & compositions)
#     """
#     inflow = np.zeros(
#         (len(
#             stock_share.columns.levels[0]), len(
#             stock.iloc[0]), len(stock)))
#     outflow_cohort = np.zeros(
#         (len(
#             stock_share.columns.levels[0]), len(
#             stock.iloc[0]), len(stock), len(stock)))
#     stock_cohort = np.zeros(
#         (len(
#             stock_share.columns.levels[0]), len(
#             stock.iloc[0]), len(stock), len(stock)))

#     stock_by_vtype = pd.DataFrame(
#         0,
#         index=stock_share.index,
...
#     # Then run the original DSM for each vehicle type & add it to the inflow, ouflow & stock containers, (for stock:
#     # index = time; columns = vtype, region)
#     for vtype in vtype_list:
...
def nr_by_cohorts_to_materials_simple_np(
        inflow,
        outflow_cohort,
        stock_cohort,
        weight,
    ...
    OUTPUT: --------------
    pd_inflow_mat  = pandas dataframe (index: material, time; column: regions) - inflow of materials in kg of material,
                     by region & time
    pd_outflow_mat = pandas dataframe (index: material, time; column: regions) - inflow of materials in kg of material,
                     by region & time
    pd_stock_mat   = pandas dataframe (index: material, time; column: regions) - inflow of materials in kg of material,
                     by region & time
    """
    zeros = np.zeros((len(composition.columns), len(inflow[0]), len(inflow)))
    inflow_mat = zeros
    outflow_mat = zeros
    stock_mat = zeros

    for material in range(0, len(composition.columns)):
        # before running, check if the material is at all relevant in the vehicle
        ...
        if composition.iloc[:, material].sum() > 0.001:
            for region in range(0, len(inflow[0])):
                ...
                inflow_mat[material, region, :] = (
                    inflow[:, region] * weight) * composition_used
                outflow_mat[material, region, :] = np.multiply(np.multiply(
                    outflow_cohort[region, :, :].T, weight), composition_used).T.sum(axis=1)
                stock_mat[material, region, :] = np.multiply(np.multiply(
                    stock_cohort[region, :, :], weight), composition_used).sum(axis=1)
        else:
            pass
    ...
    index = pd.MultiIndex.from_product(
        [composition.columns, range(END_YEAR + 1 - len(inflow),
                                    END_YEAR + 1)],
        names=['time', 'type'])
    pd_inflow_mat = pd.DataFrame(
        inflow_mat.transpose(
            0, 2, 1).reshape(
            (length_materials * length_time), REGIONS), index=index, columns=range(
                1, len(
                    inflow[0]) + 1))
    pd_outflow_mat = pd.DataFrame(
        outflow_mat.transpose(
            0, 2, 1).reshape(
...
def nr_by_cohorts_to_materials_typical_np(
        inflow,
        outflow_cohort,
        stock_cohort,
        weight,
    ...
    """
    inflow_mat = np.zeros((len(inflow), len(
        composition.columns.levels[1]), len(inflow[0]), len(inflow[0][0])))
    outflow_mat = np.zeros((len(inflow), len(
        composition.columns.levels[1]), len(inflow[0]), len(inflow[0][0])))
    stock_mat = np.zeros((len(inflow), len(
        composition.columns.levels[1]), len(inflow[0]), len(inflow[0][0])))

    # composition.columns.names = ['type', 'material']

    # get two dictionaries to keep tarck of the order of materials & vtypes
    # while using numpy
    vtype_list = list(composition.columns.unique('type'))
    # columns.unique() keeps the original order, which is important here (same
    # for materials)
    ...
    for vtype in range(0, len(vtype_list)):
        # before running, check if the vehicle type is at all relevant in the
        # vehicle (save calculation time)
                ...
                if composition_used.sum() > 0.001:
                    for region in range(0, len(inflow[0])):
                        ...
                        stock_mat[vtype, material, region, :] = \
                            np.multiply(np.multiply(stock_cohort[vtype, region, :, :], weight_used),
    ...
    index = pd.MultiIndex.from_product(
        [mater_list, list(
            range((END_YEAR + 1 - len(inflow[0][0])), END_YEAR + 1))],
        names=['material', 'time'])
    columns = pd.MultiIndex.from_product(
        [vtype_list, range(1, REGIONS + 1)], names=['type', 'region'])
    pd_inflow_mat = pd.DataFrame(
        inflow_mat.transpose(1, 3, 0, 2).reshape(
            (length_materials * length_time), (len(inflow) * REGIONS)),
        index=index, columns=columns)
    pd_outflow_mat = pd.DataFrame(
        outflow_mat.transpose(1, 3, 0, 2).reshape(
            (length_materials * length_time), (len(inflow) * REGIONS)),
        index=index, columns=columns)
    pd_stock_mat = pd.DataFrame(
        stock_mat.transpose(1, 3, 0, 2).reshape(
            (length_materials * length_time), (len(inflow) * REGIONS)),
        index=index, columns=columns)
