...
per km of grid; storage costs ($/ct/kWh) and cost corrections; grid storage technology market shares;
...
- **Storage** (``electricity.storage``): Infers energy storage capacity stocks for pumped hydropower storage (PHS),
  and grid-scale dedicated storage (e.g. LFP batteries) from IMAGE-energy power and energy storage capacity data 
...
3. Execute relevant calculations.


Output
------

The ``electricity_preprocessing`` returns a dictionary for each sub-sector with model-ready
...
* ``set_unit_flexible`` (e.g. MWh)

Simulation
==========
The electricity module uses both the `GenericStocks class <../api/models_detail.html>`_ and `SharesInflowStocks class <../api/models_detail.html>`_ for stock modelling and the `MaterialIntensities class <../api/models_detail.html>`_ for calculating material flows in simulation.
