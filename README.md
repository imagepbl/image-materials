...
#### Resource Efficiency and Circular Economy flags

Resource efficiency and circular economy (CE) measures (e.g. floorspace reduction, lifetime extension, lightweighting, recycling) are controlled by a flag-based system under `data/raw/circular_economy_scenarios/`. Each scenario is a folder containing two files that must be used together:

- `resource_efficiency_flags.toml` — booleans that switch each measure on or off, grouped by sector (`[buildings]`, `[vehicles]`, `[electricity]`, `[end_of_life]`, `[rest_of]`).
- `circular_economy_data.toml` — the parameters for every measure (target years, regional percentage changes, material intensity changes, etc.), keyed as `[sector.FlagName]` so a flag's data is always self-contained.
