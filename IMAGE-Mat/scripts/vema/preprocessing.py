...
def preprocessing(base_dir=os.getcwd()):
    ...
    nr_of_boats         = pd.read_csv(standard_input_data_path. \
                                      joinpath("ships", "number_of_boats.csv"), 
                                      index_col="t").sort_index(axis=0)           
    # number of boats in the global merchant fleet (2005-2018) changing Data by EQUASIS
    cap_of_boats        = pd.read_csv(standard_input_data_path.\
                                      joinpath("ships",
    ...
    loadfactor_boats    = pd.read_csv(standard_input_data_path.\
                                      joinpath("ships", "loadfactor_boats.csv"), 
                                      index_col="t").sort_index(axis=0)          
    # loadfactor of boats (fraction) fixed Data is based on Ecoinvent report 14 on Transport (Table 8-19)
    mileage_boats       = pd.read_csv(standard_input_data_path.\
                                      joinpath("ships", 
    ...
    weight_boats        = pd.read_csv(standard_input_data_path.\
                                      joinpath("ships", 
    ...
    lifetimes_vehicles = pd.read_csv(base_input_data_path.\
                                     joinpath(FOLDER, "lifetimes_years.csv"), 
                                     index_col=[0,1])
    ...
    kilometrage = pd.read_csv(base_input_data_path.\
                              joinpath(FOLDER, "kilometrage.csv"),
                              index_col="t")
    # kilometrage of passenger cars in kms/yr
    kilometrage_midi_bus = pd.read_csv(base_input_data_path.\
                                       joinpath(FOLDER, "kilometrage_midi.csv"),
                                       index_col="t") # kilometrage of midi-buses in kms/yr
    kilometrage_bus = pd.read_csv(base_input_data_path.\
                                  joinpath(FOLDER, "kilometrage_bus.csv"),  
                                  index_col="t")     # kilometrage of regular buses in kms/yr
    mileages = pd.read_csv(base_input_data_path.\
                           joinpath(FOLDER, "mileages_km_per_year.csv"), 
                           index_col="t") # Km/year of all the vehicles (buses & cars have region-specific files)
    
    # weight and materials related data
    vehicle_weight_kg_simple = pd.read_csv(base_input_data_path.\
                               joinpath(FOLDER,
                                        "vehicle_weight_kg_simple.csv"),   
                                        index_col=0)       # Weight of a single vehicle of each type in kg
    vehicle_weight_kg_typical = pd.read_csv(base_input_data_path.\
                                joinpath(FOLDER, 
                                         "vehicle_weight_kg_typical.csv"),  
                                         index_col=[0,1])   # Weight of a single vehicle of each type in kg
    material_fractions = pd.read_csv(base_input_data_path.\
                                     joinpath(FOLDER, 
                                              "material_fractions_simple.csv"),  
                                              index_col=[0,1])   # Material fractions in percentages
    material_fractions_type = pd.read_csv(base_input_data_path.\
                              joinpath(FOLDER,
                                       "material_fractions_typical.csv"), 
                                       index_col=[0,1], header=[0,1])   
    # Material fractions in percentages, by vehicle sub-type
    battery_weights = pd.read_csv(base_input_data_path.\
                                  joinpath(FOLDER, 
    ...
    # determined
    battery_materials = pd.read_csv(base_input_data_path.\
                                    joinpath(FOLDER, "battery_materials.csv"), 
                                    index_col=[0,1])   
    ...
    vehicle_shares_typical         = pd.DataFrame(0, index=years_range, columns=columns)
    ...
    material_fractions_simple = material_fractions.rename_axis('mode', axis=1).\
        rename_axis(['year','material'], axis=0).stack().unstack(['mode', 'material'])
    ...
    car_pkms               = car_pkms.drop([27, 28], axis=1)    # exclude region 27 & 28 (empty & global total), mind 
    ...
    first_year_vehicle_regionalized = pd.DataFrame(0, index= \
            first_year_vehicle.index, columns=total_nr_vehicles_simple.columns)
