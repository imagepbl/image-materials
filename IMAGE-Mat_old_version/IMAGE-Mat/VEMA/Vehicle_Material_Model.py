...
material_fractions_type = pd.read_csv(base_input_data_path.joinpath(FOLDER, "material_fractions_typical_wo_Trolley.csv"), index_col=[0,1], header=[0,1])   # Material fractions in percentages, by vehicle sub-type
...
def output_stock_cohort(values: pd.DataFrame, name: str):
    regions = np.arange(1, 27)
    timesteps = np.arange((2061-len(values[1])), 2061)
    timesteps_cohort = np.arange((2061-len(values[1])), 2061)
    
    cohort_mi = pd.MultiIndex.from_product([regions, timesteps_cohort, timesteps], names=['regions', 'time', 'time_cohort'])
    
    cohort = pd.Series(index=cohort_mi, data=values.flatten(), name = name)
    cohort.to_csv(f'output/{name}_by_cohort_vema.csv')
    
output_stock_cohort(air_freight_stock_coh, 'air_freight')
output_stock_cohort(ship_medium_stock_coh, 'ship_medium')
