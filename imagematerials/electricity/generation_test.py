...
lifetimes = pd.read_csv(path_external_data_scenario / 'LTTechnical_dynamic.csv', index_col=['Year','DIM_1'])        

# material compositions of electricity generation tecnologies (g/MW)
material_intensities = pd.read_csv(path_external_data_scenario / 'generation_material_intensities.csv')#.transpose()

# market shares
market_shares = pd.read_csv(path_external_data_scenario / 'generation_subtechnologies_market_shares.csv')

# 2. IMAGE/TIMER files -----------------------------------------
# Generation capacity (stock demand per generation technology) in MW peak capacity
cap_data = read_mym_df(path_image_output / 'GCap.out')
