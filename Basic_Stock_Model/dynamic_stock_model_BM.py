...
class DynamicStockModel(object):
    ...
    def __init__(self, t=None, i=None, o=None, s=None, lt=None, s_c=None, o_c=None,
                name='DSM', pdf=None, sf=None, surplus=None, surpluslt=None, surplussf=None):
        ...
        self.sf  = sf # optional
         ...
         # additions by Berend Mintjes, CML
        self.surplus = surplus # optional

        if surpluslt is not None:
            for ThisKey in surpluslt.keys():
                # If we have the same scalar lifetime, stdDev, etc., for all cohorts,
                # replicate this value to full length of the time vector
                if ThisKey != 'Type':
                    if np.array(surpluslt[ThisKey]).shape[0] == 1:
                        surpluslt[ThisKey] = np.tile(surpluslt[ThisKey], len(t))

        self.surpluslt = surpluslt # optional
        self.surplussf = surplussf # optional
        # --
...
# Additions by Berend Mintjes, CML:
    def compute_stock_driven_model_surplus(self):
        if self.s is None:
            return None, None, None     # stock-driven model needs stock as input
        
        if ((self.sf is None) and (self.lt is None)):   
            return None, None, None     # model needs to know or be able to compute sf
        
        self.compute_sf() # compute sf if not present already

        # instantiate empty arrays
        nrOfYears = len(self.t)
        self.s_c = np.zeros((nrOfYears, nrOfYears))
        self.o_c = np.zeros((nrOfYears, nrOfYears))
        self.i = np.zeros(nrOfYears)

        if self.sf[0, 0] != 0:  # else, inflow is 0
            self.i[0] = self.s[0] / self.sf[0, 0]
        
        self.s_c[:, 0] = self.i[0] * self.sf[0, 0]
        self.o_c[0, 0] = self.i[0] - self.s_c[0, 0]

        for m in range(1, nrOfYears):
            # determine inflow from mass balance
            stockDiff = self.s[m] - self.s_c[m, :].sum()
                # difference between required stock and remaining stock (sum of
                # cohorts)
            if stockDiff > 0 and self.sf[m, m] != 0:
                # in case of positive required inflow and this inflow surviving
                # this first year, compute inflow
                self.i[m] = stockDiff / self.sf[m, m]
                # else, so also in case of negative inflow, inflow is 0
            self.s_c[m::, m] = self.i[m] * self.sf[m::, m]
                # compute this cohort's current and future size
        
        # calculate outflow (outside of loop, not used for stock calculations)
        immediateOutflow = np.diag(self.i) * (1 - self.sf)
            # create diagonal matrix using inflow and same-year survival
        stockByCohortDifference = -1 * np.diff(self.s_c, 1, axis=0, prepend=0)
            # get difference between previous and this year per cohort
        previousCohortOutflow = np.tril(stockByCohortDifference, -1)
            # only values below the diagonal are relevant and should be used
        self.o_c = immediateOutflow + previousCohortOutflow
            # combine to get total outflow by cohort

        return self.s_c, self.o_c, self.i

    def compute_stock_driven_model_initialstock_surplus(self, InitialStock, SwitchTime):
        if self.s is None:
            return None, None, None     # stock-driven model needs stock as input
        ...
        self.s_c[SwitchTime-2, 0:SwitchTime-1] = InitialStock
        
        # historic inflow
        sfHistoric = self.sf[SwitchTime-2, 0:SwitchTime-1].copy()
        sfHistoric[sfHistoric == 0] = 1
        self.i[:SwitchTime-1] = InitialStock / sfHistoric

        # stock from historic inflow
        self.s_c[:, 0:SwitchTime-1] = np.einsum(
            'tc,c->tc',
            self.sf[:, 0:SwitchTime-1],
            self.i[0:SwitchTime-1]
        )

        # add historic stock series to total stock
        self.s[0:SwitchTime-1] = self.s_c[0:SwitchTime-1,:].sum(axis =1).copy()
        
        # future years:
        for m in range(SwitchTime-1, nrOfYears):
            stockDiff = self.s[m] - self.s_c[m].sum()
            if stockDiff > 0:
                if self.sf[m, m] != 0:
                    self.i[m] = stockDiff / self.s_f[m, m]
                self.s_c[m::, m] = self.i[m] * self.sf[m::, m]
        ...
        immediateOutflow = np.diag(self.i) * (1 - self.sf)
            # create diagonal matrix using inflow and same-year survival
        stockByCohortDifference = -1 * np.diff(self.s_c, 1, axis=0, prepend=0)
            # get cohort difference between previous year and this year
        previousCohortOutflow = np.tril(stockByCohortDifference, -1)
            # only values below the diagonal are relevant and should be used
        self.o_c = immediateOutflow + previousCohortOutflow
            # combine to get total outflow by cohort
    ...
    def compute_surplussf(self):
        """
        Copy of ``compute_sf`` method, but for surplus survival (``surplussf``). 
        """
        if self.surplussf is None:
            self.surplussf = np.zeros((len(self.t), len(self.t)))
            # Perform specific computations and checks for each lifetime distribution:

            if self.surpluslt['Type'] == 'Fixed': # fixed lifetime, age-cohort leaves the stock in the model year when the age specified as 'Mean' is reached.
                for m in range(0, len(self.t)):  # cohort index
                    self.surplussf[m::,m] = np.multiply(1, (np.arange(0,len(self.t)-m) < self.surpluslt['Mean'][m])) # converts bool to 0/1
                # Example: if Lt is 3.5 years fixed, product will still be there after 0, 1, 2, and 3 years, gone after 4 years.

            if self.surpluslt['Type'] == 'Normal': # normally distributed lifetime with mean and standard deviation. Watch out for nonzero values 
                # for negative ages, no correction or truncation done here. Cf. note below.
                for m in range(0, len(self.t)):  # cohort index
                    if self.surpluslt['Mean'][m] != 0:  # For products with lifetime of 0, sf == 0
                        self.surplussf[m::,m] = scipy.stats.norm.sf(np.arange(0,len(self.t)-m), loc=self.surpluslt['Mean'][m], scale=self.surpluslt['StdDev'][m])
                        # NOTE: As normal distributions have nonzero pdf for negative ages, which are physically impossible, 
                        # these outflow contributions can either be ignored (violates the mass balance) or
                        # allocated to the zeroth year of residence, the latter being implemented in the method compute compute_o_c_from_s_c.
                        # As alternative, use lognormal or folded normal distribution options.
                        
            if self.surpluslt['Type'] == 'FoldedNormal': # Folded normal distribution, cf. https://en.wikipedia.org/wiki/Folded_normal_distribution
                for m in range(0, len(self.t)):  # cohort index
                    if self.surpluslt['Mean'][m] != 0:  # For products with lifetime of 0, sf == 0
                        self.surplussf[m::,m] = scipy.stats.foldnorm.sf(np.arange(0,len(self.t)-m), self.surpluslt['Mean'][m]/self.surpluslt['StdDev'][m], 0, scale=self.surpluslt['StdDev'][m])
                        # NOTE: call this option with the parameters of the normal distribution mu and sigma of curve BEFORE folding,
                        # curve after folding will have different mu and sigma.
                        
            if self.surpluslt['Type'] == 'LogNormal': # lognormal distribution
                # Here, the mean and stddev of the lognormal curve, 
                # not those of the underlying normal distribution, need to be specified! conversion of parameters done here:
                for m in range(0, len(self.t)):  # cohort index
                    if self.surpluslt['Mean'][m] != 0:  # For products with lifetime of 0, sf == 0
                        # calculate parameter mu    of underlying normal distribution:
                        LT_LN = np.log(self.surpluslt['Mean'][m] / np.sqrt(1 + self.surpluslt['Mean'][m] * self.surpluslt['Mean'][m] / (self.surpluslt['StdDev'][m] * self.surpluslt['StdDev'][m]))) 
                        # calculate parameter sigma of underlying normal distribution:
                        SG_LN = np.sqrt(np.log(1 + self.surpluslt['Mean'][m] * self.surpluslt['Mean'][m] / (self.surpluslt['StdDev'][m] * self.surpluslt['StdDev'][m])))
                        # compute survial function
                        self.surplussf[m::,m] = scipy.stats.lognorm.sf(np.arange(0,len(self.t)-m), s=SG_LN, loc = 0, scale=np.exp(LT_LN)) 
                        # values chosen according to description on
                        # https://docs.scipy.org/doc/scipy-0.13.0/reference/generated/scipy.stats.lognorm.html
                        # Same result as EXCEL function "=LOGNORM.VERT(x;LT_LN;SG_LN;TRUE)"
                        
            if self.surpluslt['Type'] == 'Weibull': # Weibull distribution with standard definition of scale and shape parameters
                for m in range(0, len(self.t)):  # cohort index
                    if self.surpluslt['Shape'][m] != 0:  # For products with lifetime of 0, sf == 0
                        self.surplussf[m::,m] = scipy.stats.weibull_min.sf(
                            np.arange(0,len(self.t)-m),
                            c=self.surpluslt['Shape'][m],
                            loc = 0,
                            scale=self.surpluslt['Scale'][m]
                        )
            return self.surplussf
        else:
            # surplussf already exists
            return self.surplussf

    def compute_stock_driven_model_surplus_explicit(self):
        if self.s is None:
            return None, None, None, None     # stock-driven model needs stock as input
        
        if ((self.sf is None) and (self.lt is None)):   
            return None, None, None, None     # model needs to know or be able to compute sf
        
        self.compute_sf() # compute sf if not present already
    	
        surplusSurvivalDiffers = False
        if self.surplussf is None:
            if self.surpluslt is None:
                self.surplussf = self.sf.copy()
            else:
                self.compute_surplussf()
                surplusSurvivalDiffers = not np.array_equal(
                    self.sf,
                    self.surplussf,
                    equal_nan=True
                )
        ...
        self.o_c = np.zeros((nrOfYears, nrOfYears))
        self.i = np.zeros(nrOfYears)
        self.surplus = np.zeros((nrOfYears, nrOfYears))
        ...
        for m in range(1, nrOfYears):
            # Determine inflow from mass balance
            cohortsSum = self.s_c[m, :].sum()
            stockDiff = self.s[m] - cohortsSum
                # difference between required stock and remaining stock (sum of
                # cohorts)
            if surplusSurvivalDiffers:
                if stockDiff < 0: # if negative inflow would occur
                    cohortsChangeFactor = -1 * (stockDiff/cohortsSum)
                    surplusInflowVector = self.s_c[m, 0:m] * cohortsChangeFactor  # net inflow in surplus

                    self.surplus[m, 0:m] += surplusInflowVector
                        # add surplus inflow to surplus for this year
                    
                    # surplus inflow future decay:
                    surplusSurvivalZeros = self.surplussf[m, 0:m] == 0 
                    surplusSurvivalDenom = self.surplussf[m, 0:m].copy()
                    surplusSurvivalDenom[surplusSurvivalZeros] = 1
                        # to prevent division by 0
                    surplusOriginal = surplusInflowVector / surplusSurvivalDenom
                        # survival function works from inflow year, so get "original"
                        # values to compute future decay
                    surplusOriginal[surplusSurvivalZeros] = 0
                    self.surplus[m+1:, 0:m] += surplusOriginal * self.surplussf[m+1:, 0:m]
                        # from "original", compute decay of surplus inflow and add to surplus

                    self.s_c[m::, 0:m] -= self.s_c[m::, 0:m] * cohortsChangeFactor
                        # remove surplus from current and future stock
                else:
                    surplusCohortsSum = self.surplus[m, :].sum()
                    remainderNotFromSurplus = stockDiff - surplusCohortsSum
                    if remainderNotFromSurplus > 0:
                        inflowFromSurplusVector = self.surplus[m, 0:m].copy()
                            # take all surplus stock and add to this year
                        self.surplus[m::, 0:m] = 0
                            # set surplus for this and future years to 0
                        if self.sf[m, m] != 0:
                            self.i[m] = remainderNotFromSurplus / self.sf[m, m]
                                # get remainder from inflow
                    elif surplusCohortsSum != 0:
                         partFromSurplus = stockDiff / surplusCohortsSum
                         inflowFromSurplusVector = partFromSurplus * self.surplus[m, 0:m]
                            # take required stock from surplus
                         self.surplus[m::, 0:m] -= partFromSurplus * self.surplus[m::, 0:m]
                            # remove current and future surplus transferred to stock
                    else:
                        inflowFromSurplusVector = 0
                    
                    self.s_c[m, 0:m] += inflowFromSurplusVector

                    stockSurvivalZeros = self.sf[m, 0:m] == 0
                    stockSurvivalDenom = self.sf[m, 0:m].copy()
                    stockSurvivalDenom[stockSurvivalZeros] = 1
                        # to prevent division by 0
                    stockOriginal = inflowFromSurplusVector / stockSurvivalDenom
                        # survival function works from inflow year, so get "original"
                        # values to compute future decay
                    stockOriginal[stockSurvivalZeros] = 0
                    
                    self.s_c[m+1:, 0:m] += stockOriginal * self.sf[m+1:, 0:m]
                        # from "original", compute future decay and add to stock

            else: # surplus uses same survival function as stock
                if stockDiff < 0:   # if negative inflow would occur
                    cohortsChangeFactor = -1 * (stockDiff/cohortsSum)
                    self.surplus[m::, 0:m] += \
                            self.s_c[m::, 0:m] * cohortsChangeFactor
                        # add difference to surplus
                    self.s_c[m::, 0:m] -= self.s_c[m::, 0:m] * cohortsChangeFactor
                        # correct cohorts for difference so that stockDiff equals 0
                else:
                    surplusCohortsSum = self.surplus[m, :].sum()
                    remainderNotFromSurplus = stockDiff - surplusCohortsSum
                    if remainderNotFromSurplus > 0:     # if more stock is required
                        self.s_c[m::, 0:m] += self.surplus[m::, 0:m]
                        self.surplus[m::, 0:m] = 0 
                            # take all surplus stock first
                        if self.sf[m, m] != 0:
                            self.i[m] = remainderNotFromSurplus / self.sf[m, m]
                                # take remainder from inflow
                    elif surplusCohortsSum != 0:  # if surplus exists
                        partFromSurplus = stockDiff / surplusCohortsSum 
                        self.s_c[m::, 0:m] += partFromSurplus * self.surplus[m::, 0:m]
                            # add required stock to cohorts
                        self.surplus[m::, 0:m] -= partFromSurplus * self.surplus[m::, 0:m] 
                            # take required stock from surplus
        ...
        # calculate outflow (outside of loop, as it is not required for
        # stock calculations)
        immediateOutflow = np.diag(self.i) * (1 - self.sf)
            # create diagonal matrix using inflow and same-year survival
        stockAndSurplusDifference = -1 * np.diff(self.s_c + self.surplus, 1, axis=0, prepend=0)
            # compute difference of stock and surplus with previous years
        previousCohortsOutflow = np.tril(stockAndSurplusDifference, -1)
            # only values below the diagonal are relevant and should be used
        self.o_c = immediateOutflow + previousCohortsOutflow
            # combine to get total outflow by cohort

        return self.s_c, self.o_c, self.i, self.surplus

    def compute_stock_driven_model_initialstock_surplus_explicit(self,
            InitialStock, SwitchTime):
        if self.s is None:
            return None, None, None, None     # stock-driven model needs stock as input
        ...
        # historic inflow (rewritten to not use loop)
        sfHistoric = self.sf[SwitchTime-2, 0:SwitchTime-1].copy()
        sfHistoric[sfHistoric == 0] = 1
        self.i[:SwitchTime-1] = InitialStock / sfHistoric
