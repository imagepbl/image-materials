...
from const import (parse_dim, get_key, DIM1_primsec_reversed_dict, DIM2_primary_dict, 
...
from sankey_function import create_node_dict, index_mapper, convert_index_to_node_id, prepare_Sankey_lists
...
primary_energy_supply = read_mym_df(f'{path_input_data}/IMAGE_out/{scenario}/tpes_ext.out').set_index(["time", "DIM_1"])
# Primary to Secondary energy flows DIM_1:Primary, DIM_2:Secondary GJ/yr
prim_per_sec = read_mym_df(f'{path_input_data}/IMAGE_out/{scenario}/PrimPerSec.out').set_index(["time", "DIM_1", "DIM_2"])
# Final Energy in PJ/yr by region, sector, and energy carrier [NRCT, S, NECS9T](t), [28,8,10](t) # PJ
final_energy = read_mym_df(f'{path_input_data}/IMAGE_out/{scenario}/final_energy_rt.out').set_index(["time", "DIM_1", "DIM_2"])  
